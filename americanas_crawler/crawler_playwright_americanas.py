# -*- coding: utf-8 -*-
"""Crawler Americanas.com com Playwright."""
...
async def _abrir_pagina_para_pausa(page: Page, config: ConfigAmericanas, primeiro_termo: str) -> None:
    url_inicial = montar_url_busca(primeiro_termo, 1)
    print("[INÍCIO] Abrindo Americanas.com para verificação inicial...")
    try:
        await page.goto(url_inicial, wait_until="domcontentloaded", timeout=config.timeout_ms)
        await esperar_carregamento(page, timeout_ms=config.timeout_ms)
        await fechar_popups_basicos(page)
    except PlaywrightTimeoutError:
        print("        Aviso: timeout ao abrir Americanas antes da pausa. A página pode ainda estar carregando.")
    except Exception as exc:
        print(f"        Aviso: não foi possível abrir a página inicial: {_texto_curto(str(exc), 120)}")
        print("        Acesse manualmente https://www.americanas.com.br nessa janela e depois pressione ENTER.")
...
def _imprimir_cabecalho(config: ConfigAmericanas, termos: List[str]) -> None:
    _imprimir_secao("CRAWLER AMERICANAS.COM | MINI CELULARES")
